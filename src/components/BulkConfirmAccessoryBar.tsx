...
import { bulkConfirmAccessory } from "@/lib/accessory-actions";

export default function BulkConfirmAccessoryBar({
  selectedIds,
  onDone,
}: {
  selectedIds: string[];
  ...
  const [state, formAction, pending] = useActionState(bulkConfirmAccessory, {});
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-2.5">
      ...
      {pending && <span className="text-xs text-slate-500">處理中，可能需要一點時間（含歸檔）…</span>}
        ...
        >
          ...
          >
            ...
            <p className="text-slate-800 font-medium">
              確定要確認結案這 {selectedIds.length} 筆案件嗎？
            </p>
